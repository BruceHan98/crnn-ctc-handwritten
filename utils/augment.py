...
def distort(src, segment, img_type='np_array'):
...
def blur(PIL_Image, radius=1):
    blured = PIL_Image.filter(ImageFilter.BoxBlur(radius))
    return blured


def reverse(PIL_Image):
    reversed_ = ImageOps.invert(PIL_Image)
    return reversed_
